...
def submit_quiz():
    ...
    return jsonify({"score": score, "correct": correct_count, "total": total_questions})
...
def bulk_upload_questions():
    ...
    # First pass: Validate all questions and auto-create topics
    for index, question_data in enumerate(questions_data):
        try:
            ...
            # Validate content
            if (
                not question_data["question_text"]
                or not question_data["question_text"].strip()
            ):
                failed_count += 1
            ...
            # Validate options
            if (
                not isinstance(question_data["options"], list)
                or len(question_data["options"]) != 4
            ):
                failed_count += 1
            ...
            if not topic:
                # Auto-create topic with capitalized name
                topic_name = (
                    topic_slug.upper()
                    if len(topic_slug) <= 4
                    else topic_slug.capitalize()
                )
                topic = Topic(
                    name=topic_name,
                    description=f"Learn {topic_name} concepts and best practices",
                    slug=topic_slug,
                )
                db.session.add(topic)
                db.session.flush()  # Get the topic ID without committing
            ...
            return jsonify(
                {
                    ...
                    "failed": failed_count,
                    "created_topics": list(set(created_topics)),
                    "message": message,
